 ...
 */
function inspiro_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'inspiro_form_submit';
}


function inspiro_form_submit($form, &$form_state) {
  // Import database inspiro file.
  $inspiro_file = dirname(__FILE__) . '/5pika_clean.sql';
  $success = import_inspiro($inspiro_file);
 ...
 * @see demo_reset().
 */
function import_inspiro($filename) {

 ...
 */
function inspirodemo_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'inspirodemo_form_submit';
}


function inspirodemo_form_submit($form, &$form_state) {
  // Import database inspirodemo file.
  $inspirodemo_file = dirname(__FILE__) . '/5pika_demo.sql';
  $success = import_inspirodemo($inspirodemo_file);
...
/**
 * Imports a database 5pika file.
 *
 * @see demo_reset().
 */
function import_inspirodemo($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open 5pika file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
  ...
  foreach (list_tables_demo() as $table) {
    db_drop_table(substr($table,strlen($db_prefix)));
  }

  // Load data from 5pika file.
  $success = TRUE;
  $query = '';
  $new_line = TRUE;
